...
def insert_user(data, type):
    ...
    user_from_db = User.query.filter_by(email=data["email"], phone=data["phone"]).all()
    if len(user_from_db) > 0:
        return jsonify({'message': 'User with this email/phone already exists', "success": False}), 400
    ...
    try:
        ...
        return jsonify({'data': {"token": generate_token(new_user.id, new_user.type), "user_type": user.type},
                        "success": True}), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({'message': f'Error: {str(e)}', "success": False}), 500
